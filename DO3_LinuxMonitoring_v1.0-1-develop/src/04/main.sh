...
default=""
color_name=""
color_from_config=$(grep column1_background config.txt | cut -b 20-)
...
color_from_config=$(grep column1_font_color config.txt | cut -b 20-)
...
color_from_config=$(grep column2_background config.txt | cut -b 20-)
defcolor=$(color "$color_from_config" 4)
key=$(get_color_name $color_from_config)
if [ $defcolor = "default" ]
then
...
color_from_config=$(grep column2_font_color config.txt | cut -b 20-)
defcolor=$(color "$color_from_config" 3)
key=$(get_color_name $color_from_config)
if [ $defcolor = "default" ]
then
